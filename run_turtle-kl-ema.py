...
#python run_turtle.py --dataset cifar100 --phis clipvitL14 dinov2 --reg_type kl_ema --gamma 10
def _parse_args(args):
    ...
    type=str,
    default='entropy',
    choices=['entropy', 'kl_ema'],
    help='Type of regularization'
    )
    parser.add_argument('--beta', type=float, default=0.1, help='Hyperparameter for kl_ema regularization')
...
def run(args=None):
    ...
    prior_ema = torch.ones(C).to(args.device) / C
    ema_momentum = 0.99

    # Define task encoder
    if args.reg_type == "kl_article":
        task_encoder = [BayesianLinear(d, C).to(args.device) for d in feature_dims]
    else:
    ...
    for i in iters_bar:
        ...
        for l in label_per_space:
            ...
            reg = torch.special.entr(p).sum()

            if args.reg_type == 'kl_ema':
                prior = torch.clamp(prior_ema.detach(), min=1e-8)

                # KL(prior || p)
                kl_reg = torch.sum(prior * (prior.log() - p.log()))
                kl_reg_value += kl_reg
        ...
        if args.reg_type == 'kl_ema':
            with torch.no_grad():
                batch_mean = torch.stack([l.mean(0) for l in label_per_space]).mean(0)
                prior_ema = ema_momentum * prior_ema + (1 - ema_momentum) * batch_mean
                prior_ema = prior_ema / prior_ema.sum()

        beta=args.beta
        loss_total = pred_error - args.gamma * reg_value

        if args.reg_type == "kl_ema":
            loss_total += beta * kl_reg_value
        
        elif args.reg_type == "kl_article":
            loss_total += args.beta * kl_article_value
        ...
        if (i+1) % 20 == 0 or (i+1) == args.T:
            ...
            iters_bar.set_description(f'Training loss {float(pred_error):.3f}, entropy {float(reg_value):.3f}, found clusters {len(np.unique(preds_val))}/{C}, cluster acc {cluster_acc:.4f}')
    ...
    print(f'Training loss {float(pred_error):.3f}, entropy {float(reg_value):.3f}, Number of found clusters {len(np.unique(preds_val))}/{C}, Cluster Acc {cluster_acc:.4f}')
    ...
    if args.cross_val:
        ...
        del optimizer, W_in, inner_opt, pred_error, _, reg_value, labels
    ...
    task_path = f"{args.reg_type}_turtle_{phis}_innerlr{args.inner_lr}_outerlr{args.outer_lr}_T{args.T}_M{args.M}_{inner_start}_gamma{args.gamma}_bs{args.batch_size}_seed{args.seed}_beta_{args.beta}" 
    torch.save({f'phi{i+1}': task_phi.state_dict() for i, task_phi in enumerate(task_encoder)}, f'{exp_path}/{task_path}.pt')
    
    if not os.path.exists(f"{args.root_dir}/{args.reg_type}_results/{num_spaces}space/{phis}"):
        os.makedirs(f"{args.root_dir}/{args.reg_type}_results/{num_spaces}space/{phis}")

    with open(f"{args.root_dir}/{args.reg_type}_results/{num_spaces}space/{phis}/turtle_{args.dataset}_gamma{args.gamma}_beta{args.beta}.txt", 'a') as f:
        f.writelines(f"{args.reg_type} : {phis:20}, Number of found clusters {len(np.unique(preds_val))}, Cluster Acc: {cluster_acc:.4f}, Generalizatoin Score {generalization_score}, {task_path} \n")
