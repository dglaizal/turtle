...
def run(args=None):
    ...
    #calculation of the proportion of the different classes. the frequencies is gonna be used to scale the impact of the error later 
    y_gt_train = np.load(f"{args.root_dir}/labels/{args.dataset}_train.npy")
    counts = np.bincount(y_gt_train, minlength=C).astype(np.float32)
    pi = counts / counts.sum()
    pi = torch.from_numpy(pi).to(args.device)
    ...
    for i in iters_bar:
        ...
        # entropy regularization with a weighted scalar depending on the frequency of the class
        entr_reg = 0.0
        for l in label_per_space:
            p = l.mean(0)   # batch marginal
            entr_reg += torch.sum(p * torch.log((p + 1e-8) / pi))
